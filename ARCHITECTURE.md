...
| **Badge `maxUses` atomicity** | **Decision approved:** the Admin Firestore report/idempotency transaction also validates and consumes a Firestore-managed badge use for each genuinely new report. | Invalid, expired, mismatched, or exhausted badges create a pending report without consuming a use. Existing idempotent retries return before badge consumption. If the durable transaction is unavailable, report submission fails closed with `DURABLE_IDEMPOTENCY_UNAVAILABLE`. Environment-configured trusted codes remain outside Firestore badge accounting. |
...
| **Confirmation Firestore fallback to RAM** | **Decision approved:** Firestore report confirmation returns a discriminated result. Only `no_db` uses the process-local development ledger; authoritative Firestore errors return `503` and never mutate RAM, broadcast, or claim success. Missing reports return `404`; duplicate voters return `409`. | This prevents transient persistence failures from being presented as durable consensus. A durable degraded/offline confirmation ledger would require a separate lifecycle and reconciliation policy. |
