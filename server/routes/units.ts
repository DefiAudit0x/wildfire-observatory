...
router.post("/", requireRole("superadmin", "admin"), async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existing) { res.status(409).json({ error: "Unit code already exists" }); return; }
    const now = new Date().toISOString(); const actor = (req as any).admin?.agentId || "admin";
    const unit = { id: unitId, code: parsed.data.code, nameAr: parsed.data.nameAr, nameFr: parsed.data.nameFr, wilaya: parsed.data.wilaya, createdAt: now, updatedAt: now };
    const result = await createDocIfAbsent("units", unitId, unit);
    if (result === "exists") { res.status(409).json({ error: "Unit code already exists" }); return; }
    if (result === "unavailable") { res.status(503).json({ error: "Database not available" }); return; }
    logger.info({ unitId, code: unit.code, actor }, "Unit created"); res.status(201).json(unit);
  } catch (err) { logger.error({ err }, "Failed to create unit"); res.status(500).json({ error: "Internal server error" }); }
});

router.put("/:id", requireRole("superadmin", "admin"), async (req: Request, res: Response) => {
  const id = str(req.params.id); const parsed = unitSchema.safeParse(req.body);
  if (!parsed.success) { res.status(400).json({ error: "Validation failed", details: parsed.error.flatten() }); return; }
  if (toUnitId(parsed.data.code) !== id) { res.status(409).json({ error: "Unit code cannot change (it fixes the document id)" }); return; }
  try { const existing = await docGet("units", id); if (!existing) { res.status(404).json({ error: "Unit not found" }); return; } const updated = { ...existing, code: parsed.data.code, nameAr: parsed.data.nameAr, nameFr: parsed.data.nameFr, wilaya: parsed.data.wilaya, updatedAt: new Date().toISOString() }; const ok = await docSet("units", id, updated); if (!ok) { res.status(503).json({ error: "Database not available" }); return; } res.json(updated); }
  catch (err) { logger.error({ err }, "Failed to update unit"); res.status(500).json({ error: "Internal server error" }); }
});
